  ...
  return (
    <section className="py-20 bg-paper">
      <div className="container mx-auto px-6">
        <h2 className="text-3xl font-bold font-noto-serif mb-12 text-center">News</h2>
          ...
          <ul className="space-y-6">
            {newsItems.map((item, index) => (
              <li key={index} className="border-b border-gray-300 pb-6">
                <Link
                  href="#"
                  className="flex flex-col sm:flex-row sm:items-center gap-4 group hover:opacity-70 transition-opacity"
                >
                  <span className="text-sm text-gray-600 font-medium shrink-0">{item.date}</span>
                  <span className="text-base text-ink line-clamp-1">{item.title}</span>
                </Link>
              </li>
            ))}
          </ul>
